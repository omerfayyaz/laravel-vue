<template>
    <form @submit.prevent="submitLogin">
        ...
        <div class="flex items-center justify-end mt-4">
            <button class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray transition ease-in-out duration-150 ml-4" :class="{ 'opacity-25': processing }" :disabled="processing">Login</button>
        </div>
    </form>
</template>
...
<script setup>
    ...
    const { submitLogin, validationErrors, processing, loginForm } = useAuth();
